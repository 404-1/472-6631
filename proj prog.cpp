...
{ 
	...
	cout << "Press 0 to run analysis from image \nPress 1 to run analysis from camera\n\n";
	cin >> in_out;
		...
		if (in_out == 1) {
			...
			allocate_image(rgb1);
				...
				while (1) {
					...
					// process image
					p = rgb1.pdata;
					for (i = 0; i < size; i++) { // access each pixel i

						// the blue target of our example
						// (saved in "output1.bmp")
						// it had blue pixels of R = 30, G = 50, B = 150
						...
						// 3 parameters to tune -- 65,40,40
						// lighting might affect it -> maybe scale
						// the image first or measure the ambient light

						if ((B > 100) && (R < 80) && (G < 80)) {
							// set blue pixels to pure blue
							// to highlight them
							R = 0;
							G = 0;
							B = 255;
